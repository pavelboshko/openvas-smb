...
static NTSTATUS dcerpc_pipe_connect_ncacn_np_smb2_recv(struct composite_context *c)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(0,("failed NT status (%08x) in dcerpc_pipe_connect_ncacn_np_smb2_recv\n", status));
	}
...
static NTSTATUS dcerpc_pipe_connect_ncacn_unix_stream_recv(struct composite_context *c)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(0,("failed NT status (%08x) in dcerpc_pipe_connect_ncacn_unix_stream_recv\n", status));
	}
...
static NTSTATUS dcerpc_pipe_connect_ncalrpc_recv(struct composite_context *c)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(0,("failed NT status (%08x) in dcerpc_pipe_connect_ncalrpc_recv\n", status));
	}
				  ...
				  struct dcerpc_pipe **pp)
{
	...
	status = composite_wait(c);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(0,("failed NT status (%08x) in dcerpc_pipe_connect_recv\n", status));
	}
					  ...
					  struct dcerpc_pipe **p2)
{
	NTSTATUS status = composite_wait(c);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(0,("failed NT status (%08x) in dcerpc_secondary_connection_recv\n", status));
	}
